...
class FaceSaver:
    ...
    pero sólo si cumple condiciones: 
    - si require_frontal es True, debe estar orientada frontalmente,
    - y siempre debe ser nueva según histogramas.
    Además, mantiene una lista de previews en memoria para iterar sobre ellas.
    """
    def __init__(self, output_dir="faces", hist_threshold=0.9, require_frontal=True, preview_size=(200, 200)):
        self.output_dir = output_dir
        self.hist_threshold = hist_threshold  # Threshold for histogram correlation
        self.require_frontal = require_frontal  # Flag to require frontal faces
        # Create the directory if it doesn't exist
        os.makedirs(self.output_dir, exist_ok=True)
        # List to store known face histograms
        self.known_histograms = []
        # List to store preview images (resized) for display
        self.previews = []
        # Fixed size for previews (width, height)
    ...
    def save(self, frame, boxes, orientation_checker):
        ...
        sólo guarda las caras que cumplan las condiciones:
        - si require_frontal es True, la cara debe estar orientada frontal
        - la cara debe ser nueva según comparación histográfica
        Además, almacena una versión reducida para mostrar en el feed.
        """
        frame_h, frame_w, _ = frame.shape
        for (x, y, w, h) in boxes:
            # Si se requiere frontal, comprobamos orientación
            if self.require_frontal and not orientation_checker.is_facing_camera(frame, (x, y, w, h)):
                continue

            # Clamp coordinates a límites de la imagen
            x1 = max(0, x)
            y1 = max(0, y)
            x2 = min(x + w, frame_w)
            y2 = min(y + h, frame_h)
            ...
            # Si es una cara nueva (y frontal si se requiere): guardamos
            ...
            # Create and store a preview (resized) for display above el feed
            preview = cv2.resize(face_img, self.preview_size)
...
class FaceBoxApp:
    ...
    def __init__(self, webcam, detector, drawer, orientation_checker, saver):
        ...
        self.saver = saver
        # Variables para el ciclo de previews
        self.last_switch_time = time.time()
        self.current_preview_index = 0
        # Switch interval in seconds (100ms)
        self.switch_interval = 0.1

    def run(self):
        """Main loop: read frame, detect faces, guardar según configuraciones, draw boxes y display con previews"""
        try:
            while True:
                frame = self.webcam.read()
                boxes = self.detector.detect(frame)

                # Guardar según bandera require_frontal y unicidad
                if boxes:
                    self.saver.save(frame, boxes, self.orientation_checker)

                # Overlay preview over the feed (centered and with border)
                frame_with_preview = self._overlay_preview(frame)

                # Debug: print number of faces detected
                print(f"Detected {len(boxes)} face(s) in current frame", flush=True)
                self.drawer.draw(frame_with_preview, boxes)

                cv2.imshow("Face Boxes", frame_with_preview)
    ...
    def _overlay_preview(self, frame):
        """
        If there are previews stored in self.saver.previews, cycle through them every 100ms
        and overlay the current preview image centered both horizontally and vertically.
        A green border is drawn around the preview.
        """
        previews = self.saver.previews
        if not previews:
            return frame

        now = time.time()
        # If more than switch_interval has passed, advance the index
        if now - self.last_switch_time >= self.switch_interval:
            self.current_preview_index = (self.current_preview_index + 1) % len(previews)
            self.last_switch_time = now

        preview_img = previews[self.current_preview_index]
        ph, pw, _ = preview_img.shape
        fh, fw, _ = frame.shape
        ...
        # Overlay: copy pixels of preview onto frame
        roi = frame[y_offset:y_offset+ph, x_offset:x_offset+pw]
        if roi.shape[0] == ph and roi.shape[1] == pw:
            frame[y_offset:y_offset+ph, x_offset:x_offset+pw] = preview_img
            # Draw green border around the preview box
            border_color = (0, 255, 0)  # Green
            border_thickness = 2
            top_left = (x_offset, y_offset)
            bottom_right = (x_offset + pw, y_offset + ph)
...
if __name__ == "__main__":
    try:
        ...
        # require_frontal=True para exigir contacto visual; False para guardar todas las caras
        # preview_size aumentado a (200,200) para mostrar previews más grandes
        saver = FaceSaver(output_dir="faces", hist_threshold=0.9, require_frontal=True, preview_size=(200, 200))
